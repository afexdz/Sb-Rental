...
import { money, booked, financials, agencyBalances } from '../../src/admin/model'
...
async function fixture() {
  ...
  const vehicle = await insert('vehicles', { agency_id: agency.id, brand: `TEST Peugeot ${agency.id.slice(0,8)}`, model: '208', category: 'city', daily_price_cents: 500000, photos: ['/images/coastal-drive.jpg'], year: 2026 }, vehicles)
  const otherVehicle = await insert('vehicles', { agency_id: agency.id, brand: `TEST Renault ${agency.id.slice(0,8)}`, model: 'Clio', category: 'compact', daily_price_cents: 450000, active: false, availability: 'maintenance' }, vehicles)
  ...
  const conversation = await insert('conversations', { client_id: customer.id, agency_id: agency.id, reservation_id: reservation.id, subject: `TEST Livraison à Alger ${customer.id.slice(0,8)}` }, conversations)
  ...
  return { admin, customer, agency, outsider, vehicle, otherVehicle, reservation, pendingReservation, deposit, conversation }
}
...
test('base vide : zéro finance, huit sections, états vides, navigation responsive et accès direct', async ({ page }, info) => {
  ...
  // Simulate empty business tables without erasing the database.
  await page.route(/\/rest\/v1\/(vehicles|reservations|payments|conversations|conversation_messages|conversation_admin_reads|admin_audit_logs)\?/, route => route.fulfill({status:200,contentType:'application/json',body:'[]'}))
  ...
  await page.reload()
  await expect(page.getByRole('heading', { name: 'Statistiques indisponibles' })).toBeVisible()
  await expect(page.getByRole('region', { name: 'Statistiques', exact: true })).toHaveCount(0)
  await expect(page.getByRole('alert')).toContainText('vehicles')
  ...
  await expect(page.getByRole('heading',{name:'Aucun véhicule enregistré'})).toHaveCount(0)
...
test('données réelles de test : filtres, détails, finances exactes, CSV, messages et journal', async ({ page }, info) => {
  ...
  const allBookings = (await service.from('reservations').select('*')).data!
  const allPayments = (await service.from('payments').select('*')).data!
  const totals = financials(allPayments), balances = agencyBalances(allPayments,allBookings)
  ...
  await expect(stats.nth(0).locator('strong')).toHaveText(money(booked(allBookings)))
  await expect(stats.nth(1).locator('strong')).toHaveText(money(totals.commission))
  ...
  await page.goto('/admin/vehicules')
  await page.getByRole('searchbox').fill(f.otherVehicle.brand)
  await expect(page.getByRole('row').filter({ hasText: f.otherVehicle.brand })).toBeVisible()
  await page.getByRole('searchbox').fill(f.vehicle.brand)
  await expect(page.getByRole('row').filter({ hasText: f.otherVehicle.brand })).toHaveCount(0)
  await page.getByRole('link', { name: 'Consulter' }).click()
  await expect(page.getByRole('heading', { name: `${f.vehicle.brand} 208` })).toBeVisible()
  await expect(page.getByRole('img', { name: `${f.vehicle.brand} 208, photo 1` })).toBeVisible()
  ...
  await expect(finance.nth(0).locator('strong')).toHaveText(money(totals.paid))
  await expect(finance.nth(3).locator('strong')).toHaveText(money(balances.payable))
  await expect(finance.nth(4).locator('strong')).toHaveText(money(totals.pending))
  await expect(finance.nth(5).locator('strong')).toHaveText(money(totals.refunded))
  ...
  await page.getByRole('searchbox').fill(f.conversation.subject)
  await expect(page.getByRole('row').filter({ hasText: f.conversation.subject })).toBeVisible()
  await page.getByRole('link', { name: f.conversation.subject }).click()
  ...
  await page.goto('/admin/messages')
  await page.getByRole('searchbox').fill(f.conversation.subject)
  await expect(page.getByRole('row').filter({hasText:f.conversation.subject}).getByRole('cell').nth(4)).toHaveText('—')
  await page.goto('/admin/journal')
  await page.getByRole('searchbox').fill(f.admin.email)
  await expect(page.getByRole('region', { name: 'Journal administrateur', exact: true }).getByText('Réservation modifiée', { exact: true }).first()).toBeVisible()
  await expect(page.getByRole('region', { name: 'Journal administrateur', exact: true }).getByText('Paiement modifié', { exact: true }).first()).toBeVisible()
...
test('RLS, paiements non falsifiables, transitions et audit non forgeable', async () => {
  ...
  for (const table of ['vehicles', 'reservations', 'payments', 'conversations', 'conversation_messages', 'admin_audit_logs']) {
    if (table === 'vehicles') expect((await f.outsider.api.from(table).select('*').eq('agency_id',f.agency.id)).data).toEqual([])
    else expect((await f.outsider.api.from(table).select('*')).data).toEqual([])
