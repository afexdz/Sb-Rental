...
import { validateAuth, validateAgency, authErrorMessage, normalizeRcNumber, isObfuscatedSignup, agencyRequestErrorMessage, duplicateEmailMessage, duplicateRcMessage } from '../src/lib/auth.ts'
...
test('erreurs Auth traduites sans divulguer les messages techniques', () => {
  ...
  assert.equal(authErrorMessage({ code: 'user_already_exists' }), duplicateEmailMessage)
  assert.equal(authErrorMessage({ code: 'email_exists' }), duplicateEmailMessage)
...
test('RC : formats équivalents et erreurs ciblées, sans détails SQL', () => {
  assert.equal(normalizeRcNumber(' 16 / b-2026.001 '), '16B2026001')
  assert.equal(normalizeRcNumber('１６－ｂ／２０２６．００１'), '16B2026001')
  assert.equal(agencyRequestErrorMessage({ code: '23505', message: 'duplicate key violates unique constraint "agency_requests_rc_normalized_key"' }), duplicateRcMessage)
  assert.doesNotMatch(agencyRequestErrorMessage({ code: '23505', message: 'agency_requests_profile_id_key' }), /autre agence/)
})
test('inscription : reconnaît les comptes masqués sans session', () => {
  assert.equal(isObfuscatedSignup({ identities: [] }), true)
  assert.equal(isObfuscatedSignup(null), true)
  assert.equal(isObfuscatedSignup({ identities: [{ provider: 'email' }] }), false)
})
test('correction : conserve le document existant et valide tout remplacement', () => {
  const agency = { ...fields, businessName: 'Agence Test', rcNumber: 'RC-123', rcFile: null }
  assert.deepEqual(validateAgency(agency, true), {})
  assert.ok(validateAgency(agency).rcFile)
  assert.ok(validateAgency({ ...agency, rcFile: new File(['x'], 'rc.html', { type: 'text/html' }) }, true).rcFile)
})
