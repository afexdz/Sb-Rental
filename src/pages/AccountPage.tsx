...
function AccountAccess({ userId, email }: { userId: string; email: string }) {
  ...
  return <AccountLayout title="Mon compte"><p className="eyebrow">VOTRE ESPACE</p><h1>Mon <em>compte.</em></h1>
    ...
    {access && ['pending', 'rejected', 'needs_changes'].includes(access) && <Link className="button" to="/inscription?profil=agence">Corriger mon dossier <ArrowUpRight size={18} /></Link>}
...
function AccountDetails({ userId, email }: { userId: string; email: string }) {
  ...
  return <AccountLayout title="Mon compte"><p className="eyebrow">VOTRE ESPACE CLIENT</p><h1>Mon <em>compte.</em></h1><p className="account-intro">Vos informations, à portée de main.</p>
    ...
    <ClientProfile userId={userId} email={email} /><Marketplace userId={userId} /><Chat userId={userId} role="client" /><Link className="account-explore" to="/#vehicules"><span>Votre prochaine escapade ?<strong>Découvrez nos véhicules</strong></span><ArrowUpRight size={23} /></Link></>}
